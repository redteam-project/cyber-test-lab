...
class MirrorCrawlerDispatcher(object):
  ...
  def dispatch_pods(self, mirrors):
    self.run_command('/usr/local/bin/kube.sh', 'kube.sh')
    ...
    for mirror in mirrors:
      job_name = mirror.replace('http://', '')\
        .replace('https://', '')\
        .replace('/', '-')\
        .replace('.', '-')\
        .replace('_', '-')\
        .rstrip('-')
      yaml_file = self.config['local_path'] + '/job.yaml'
      with open(yaml_file, 'w') as f:
        f.write(Template(j2_template).render(job_name=job_name,
                                             mirror=mirror))
      try:
        results = self.run_command('/usr/bin/kubectl create -f ' + yaml_file,
                                   'kubectl create')
        os.unlink(yaml_file)
